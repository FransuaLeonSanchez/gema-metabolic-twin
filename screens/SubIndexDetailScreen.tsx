...
import { subIndexHistory, subIndexInfo, subIndexReal, subIndices } from "@/lib/mockData";
import { subIndexRisk } from "@/lib/icm";
  ...
  return (
    <div className="h-full overflow-y-auto scroll-hide px-5 pt-2 pb-[100px]">
      ...
      <div className="flex items-center gap-4">
        <ScoreRing value={idx.value} color={risk.color} size={120} label="puntos" />
        <div className="flex-1">
          <div className="flex flex-wrap items-center gap-1.5 mb-2">
            <div
              className="inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-[11px] font-bold"
              style={{ backgroundColor: `${risk.color}1f`, color: risk.color }}
            >
              <Icon size={12} /> {risk.label}
            </div>
            <div className="inline-flex items-center px-2 py-1 rounded-full text-[11px] font-bold bg-black/[0.035] text-sub">
              peso {idx.weight}% del ICM
            </div>
          </div>
      ...
      {real && (
        <Card className="mt-4" accent={risk.color}>
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-sub text-[11px] uppercase tracking-wider font-bold">
                {real.label}
              </p>
              <p className="text-txt text-[22px] font-extrabold mt-0.5 tabular-nums">
                {real.value}
              </p>
            </div>
            <p className="text-hint text-[11px] text-right leading-snug max-w-[45%]">
              {real.sub}
            </p>
          </div>
        </Card>
      ...
      <Card className="mt-3">
        ...
        <Sparkline data={history} color={risk.color} showAxis={false} height={90} />
